...
启航电商 ERP（`qihang-erp-open`）定位为电商企业的“订单中台 + 业务中台”底座，面向多平台、多店铺的订单履约场景，重点解决商品、订单、售后、发货、库存与系统管理一体化协同问题。项目官方描述其核心能力包含：多平台订单聚合、售后处理、库存管理与开放接口对接。
...
- 前端：Vue2 + Element UI（`vue2/` 为升级版前端，`vue/` 为旧版已过时）。
...
前端项目有两个目录：`vue2/`（升级版，Vue CLI 5 + Webpack 5 + Vue 2.7.16）和 `vue/`（旧版，Vue CLI 4 + Webpack 4 + Vue 2.6.12，已过时）。新功能开发请使用 `vue2/`。
