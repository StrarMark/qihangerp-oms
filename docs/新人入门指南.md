...
> 适用对象：第一次接触 `qihang-erp-open` 的后端 / 前端 / 测试同学。  
...
- 前端：Vue2 + Element UI（`vue2/` 为升级版前端，`vue/` 已过时）
...
├─ vue2/              # 前端工程升级版（Vue CLI 5 + Webpack 5 + Vue 2.7.16）
├─ vue/               # 前端工程旧版（Vue CLI 4 + Webpack 4 + Vue 2.6.12，已过时）
└─ docs/              # 文档与数据库脚本
```
...
> 注意：`vue2/` 为升级版前端（Vue CLI 5 + Webpack 5），`vue/` 为旧版（已过时）。新功能请使用 `vue2/`。
