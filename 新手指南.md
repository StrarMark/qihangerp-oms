...
## 1. 项目简介
- 仓库：qihang-erp-open（后端多模块 + 前端 Vue 项目）
...
- 前端（vue2/）：Vue 2.7.16 + ElementUI 2.15.13 + Vue CLI 5 + Webpack 5，新增 vant 移动端组件、vue-i18n 国际化、marked Markdown 渲染、@paypal/paypal-js 支付等能力
- 前端（vue/）：旧版（Vue CLI 4 + Webpack 4 + Vue 2.6.12，已过时）

注意：oms-api 依赖本地 libs/open-sdk-2.1.12.jar（systemPath 方式），建议在 CI/CD 中明确提供或上传到私服后改为普通依赖。

## 3. 模块结构概览
- 顶层聚合：qihang-erp-open（pom packaging）
  - api（微服务）
  ...
  - vue2：前端升级版（Vue CLI 5 + Webpack 5 + Vue 2.7.16，含 vant/vue-i18n 等新能力）
  - vue：前端旧版（Vue CLI 4 + Webpack 4 + Vue 2.6.12，已过时，请使用 vue2/）

依赖关系（自上而下）：api -> service -> interfaces/mapper -> model；所有模块共享 core/common & core/security。

## 4. 环境准备
- 必备：JDK 17、Maven 3.8+、Node.js（vue2/ 支持 14.x~22.x，推荐 20+）、MySQL 8、Redis 7、Nacos 2.3.x
- 可选：Sentinel 1.8.7
...
3) 启动前端
- 切换到 vue2 目录（`vue/` 已过时），`npm install`，`npm run dev`
...
- mapper/：DAO 与 XML 映射
- service/：业务实现
- vue2/：前端升级版工程（Vue CLI 5 + Webpack 5 + Vue 2.7.16，新增 vant、vue-i18n 等组件）
- vue/：前端旧版工程（Vue CLI 4 + Webpack 4 + Vue 2.6.12，已过时）
- docs/：SQL/预览图/文档
