    ...
    <el-table v-loading="loading" :data="locationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" align="center" prop="warehouseType">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type === 1" size="small">本地仓</el-tag>
          <el-tag v-else-if="scope.row.type === 2" size="small">云仓</el-tag>
          <el-tag v-else size="small">未知仓库</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="仓库类型" align="center" prop="warehouseType">
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.warehouseType === 'CLOUD'" size="small">云仓库</el-tag>
          <el-tag v-else-if="scope.row.warehouseType === 'JDYC' && scope.row.jdlApiType==0" size="small">京东云仓-仓配一体</el-tag>
          <el-tag v-else-if="scope.row.warehouseType === 'JDYC' && scope.row.jdlApiType==1" size="small">京东云仓-ERP</el-tag>
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status == 0" size="small">禁用</el-tag>
          <el-tag v-if="scope.row.status == 1" size="small">启用</el-tag>
          <el-tag v-if="scope.row.status == 2" size="small">正常</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item v-if="form.warehouseType==='JDYC'" label="AppKey" prop="appKey">
          <el-input v-model="form.appKey" style="width: 300px" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType==='JDYC'" label="AppSecret" prop="appSecret">
          <el-input v-model="form.appSecret" style="width: 300px" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType==='JDYC'" label="AccountToken" prop="accountToken">
          <el-input v-model="form.accountToken" style="width: 300px" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType==='JDYC'" label="RefreshToken" prop="accountToken">
          <el-input v-model="form.refreshToken" style="width: 300px" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType==='JDYC'" label="Pin" prop="bizPin">
          <el-input v-model="form.bizPin" style="width: 300px" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType==='JDYC'" label="事业部编码" prop="bizId">
          <el-input v-model="form.bizId" style="width: 300px" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType==='JDYC'" label="来源编号" prop="sourceNo">
          <el-input v-model="form.sourceNo" style="width: 300px" placeholder="来源编号，ISV开头" />
        </el-form-item>
        <el-form-item v-if="form.warehouseType!=='JDYC'" label="省市区" prop="provinces">
          <el-cascader
