  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="taoRefundList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" align="center" prop="type">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.type === 11" size="small"> 仅退款</el-tag>
          ...
          <el-tag v-if="scope.row.type === 40" size="small"> 上门服务</el-tag>
          <el-tag v-if="scope.row.type === 80" size="small"> 补发商品</el-tag>
          <el-tag v-if="scope.row.type === 90" size="small"> 补款</el-tag>
          <el-tag v-if="scope.row.type === 91" size="small"> 返现</el-tag>
      ...
      <el-table-column label="售后阶段" align="center" prop="refundPhase">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.refundPhase === 'AFTER_SALE'" size="small">售后</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="售后状态" align="center" prop="status">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.status === 1" size="small"> 售后关闭</el-tag>
          <el-tag v-if="scope.row.status === 2" size="small">售后处理中</el-tag>
          <el-tag v-if="scope.row.status === 3" size="small">退款中</el-tag>
          <el-tag v-if="scope.row.status === 4" size="small">售后成功</el-tag>
          <el-tag v-if="scope.row.status === 5" size="small">待用户处理</el-tag>
          <el-tag v-if="scope.row.status === 6" size="small">待买家发货</el-tag>
          <el-tag v-if="scope.row.status === 8" size="small">平台处理中</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="处理状态" align="center" prop="confirmStatus">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.confirmStatus === 1" size="small"> 已确认</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <!--        </el-form-item>-->
        <el-form-item v-if="form.type == 80" label="收件人" prop="receiverName">
          <el-input v-model="form.receiverName" placeholder="收件人" />
        </el-form-item>
        <el-form-item v-if="form.type == 80" label="收件手机号" prop="receiverTel">
          <el-input v-model="form.receiverTel" placeholder="收件手机号" />
        </el-form-item>
        <el-form-item v-if="form.type == 80" label="收件地址" prop="receiverAddress">
    ...
    <el-dialog title="手动添加售后" :visible.sync="addOpen" width="1000px" append-to-body>
      <el-form ref="addForm" :model="addForm" :rules="addRules" label-width="140px">
        ...
        <el-form-item label="店铺订单" prop="orderId">
          ...
          <el-table ref="singleTable" :data="orderItemList" highlight-current-row @current-change="handleCurrentChange" @row-click="handleRowClick">
            ...
            <el-table-column label="" prop="title" width="260" />
            <el-table-column label="" prop="skuName" />
            <el-table-column label="" prop="skuCode" />
            <el-table-column label="" prop="quantity" width="60" />
        ...
        <el-row>
          <el-form-item label="退款金额" prop="refundAmount">
            <el-input v-model.number="addForm.refundAmount" type="number" placeholder="请输入退款金额" style="width: 300px;" />
          </el-form-item>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-form-item v-if="addForm.hasGoodReturn==1" label="退货物流" prop="returnLogistics">
        ...
        <el-row>
          <el-form-item label="备注" prop="remark">
            <el-input v-model="addForm.remark" type="textarea" style="width: 300px;" placeholder="请输入备注" />
          </el-form-item>
        </el-row>
    ...
    <!-- 添加品对话框 -->
    <PopupShopOrderItemList ref="popup" :btn="2" @data-from-select="handleDataFromPopup" />
  </div>
</template>
